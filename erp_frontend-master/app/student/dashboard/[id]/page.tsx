...
import Result from "@/app/components/Graphs/Result";
interface Props {
  params: {
    id: number;
  };
...
export default function Page(props: Props) {
  ...
  let {
    ...
    queryKey: ["result-graph-data"],
    queryFn: () => {
      return axios.get(
        `${process.env.NEXT_PUBLIC_DOMAIN}/backend-api/result/result-data`,
        {
          withCredentials: true,
        }
      );
  ...
  return (
    <main className="w-[100vw] min-h-[100vh] bg-white  ">
      ...
      <div className="flex flex-col lg:flex-row px-5  mt-[-50px]   z-10 h-auto  overflow-hidden ">
        <div className="lg:w-[70%] h-full flex flex-col gap-2 gap-y-3 lg:mr-2 ">
